...
export default function HoverSwellMenu({
  ...
  return (
    <div className='flex h-[50rem] w-full flex-col divide-y-2 divide-black'>
      {items.map((item) => (
        <Link
          href={`/projects?project=${encodeURIComponent(item.title)}`}
        ...
        >
          ...
          <div className='absolute inset-0 z-10 w-full bg-teal-100 transition-all duration-700 group-hover:translate-x-full'></div>
          <div className='relative z-20 flex h-full flex-col items-center p-4 md:flex-row'>
            <h3 className='basis-1/3 whitespace-nowrap px-12 text-[clamp(1rem,2vw,4rem)] text-teal-400'>
              {item.title}
            </h3>
            <div className='relative basis-2/3 text-[clamp(1rem,2vw,4rem)]'>
              <p className='font-medium transition-colors duration-500 group-hover:text-teal-400'>
                {item.subtitle}
              </p>
              <div className='absolute top-full h-fit overflow-y-hidden'>
                <div className='flex gap-x-4 text-teal-400'>
